...
def _is_garbage_line(line: str) -> bool:
    ...
    if len(line) < 15 and ("/" in line or "\\" in line):
        return True

    if len(line) < 20:
        readable_count = sum(1 for char in line if char.isalnum() or char.isspace())
        if readable_count / len(line) < 0.5:
            return True
...
def extract_text_from_pdf(pdf_path: str) -> str:
    ...
    with pdfplumber.open(pdf_file) as pdf:
        ...
        for i, page in enumerate(pdf.pages):
            ...
            # Prefer native PDF text when it is substantial; otherwise use OCR.
